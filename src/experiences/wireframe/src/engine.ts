import { setForeground } from "../../../shared"

const memoize = <T>(factory: (key: number) => T) => {
...
export const StartYourEngines = () => {
  ...
  const getMemoizedRandomRatio = memoize((num: number): number => {
  ...
  const getMemoizedRandomChange = memoize((num: number): number => getRandInt(0, 360));
